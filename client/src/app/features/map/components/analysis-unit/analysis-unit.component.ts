...
export class AnalysisUnitComponent implements OnInit, OnDestroy {
  ...
  singlePointMonotemporalAnalysis: {verbose_name: string, data: {value: any, units: any}}[] = [];
  ...
  performPointAnalysis(){
    ...
    for(let [i, layer] of this.layers.entries()){
      ...
      dataPromise.then((data) => {
        let dataPoint = {
          verbose_name: layer.model.data.verbose_name,
          data: data
        }
        ...
        dataPromise.then((data) => {
          let plot = {
            ...
            data: [{
              ...
              type: "scatter",
              mode: 'lines+points', 
              name: layer.model.data.verbose_name + " (" + layer.model.data.units + ")",

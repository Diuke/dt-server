                ...
                this.mediator.getData(layer, evt.coordinate, params).then(data => {
                  if(data){
                    this.dataPreviewPopupContent.push(
                      {layer: layer.data.verbose_name + ": ", value: data.value + " " + data.units}
                    );
                  } else {
                    this.dataPreviewPopupContent.push(
                      {layer: "No data for layer: " + layer.data.verbose_name}
                    );
                  }
  ...
  filterLayers(){
    ...
    for(let element of filteredLayers){
      let layers = element["layers"].filter((layer: any) => {        
        return layer.verbose_name.toUpperCase().includes(filterName);
      })
